...
public class RegistrationFormController {
    ...
    @FXML
    private void contrôle(ActionEvent event){
        if (entraineurCheckBox.isSelected()) {
            adherentCheckBox.setDisable(true);
            entraineurCheckBox.setDisable(false);
        } else if (adherentCheckBox.isSelected()) {
            entraineurCheckBox.setDisable(true);
            adherentCheckBox.setDisable(false);
        }
        else if (!adherentCheckBox.isSelected()) {
            entraineurCheckBox.setDisable(false);
            adherentCheckBox.setDisable(false);
                ...
                try (PreparedStatement utilisateurStatement = connection.prepareStatement(utilisateurInsertSql, Statement.RETURN_GENERATED_KEYS)) {
                    ...
                    if (utilisateurAffectedRows > 0) {
                        ...
                        if (utilisateurKeys.next()) {
                            ...
                            if (entraineurCheckBox.isSelected()) {
                                // Insert data into the entraineur table
                                String entraineurInsertSql = "INSERT INTO entraineur (EntraineurID, Nom, Prenom, Email, MotDePasse, Telephone, Specialite) VALUES (?, ?, ?, ?, ?, ?, ?)";
                                try (PreparedStatement entraineurStatement = connection.prepareStatement(entraineurInsertSql)) {
                                    entraineurStatement.setInt(1, utilisateurID);
                                    entraineurStatement.setString(2, nomField.getText());
                                    entraineurStatement.setString(3, prenomField.getText());
                                    entraineurStatement.setString(4, emailField.getText());
                                    entraineurStatement.setString(5, motDePasseField.getText());
                                    entraineurStatement.setString(6, telephoneField.getText());
                                    entraineurStatement.setString(7, "Default Specialite"); // Set your default value for Specialite

                                    int entraineurAffectedRows = entraineurStatement.executeUpdate();
                                    if (entraineurAffectedRows > 0) {
                                        showRegistrationSuccessAlert();
                                        clearFields();
                                    } else {
                                        showRegistrationErrorAlert1("Entraineur registration failed.");
                                    }
                                }
                            } else {
                                // Insert data into the adherent table
